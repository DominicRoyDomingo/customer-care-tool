...
class UserStatusController extends Controller
{
    ...
    public function getDeactivated(ManageUserRequest $request)
    {
        ...
        return view('backend.auth.user.deactivated')
            ->withUsers($this->userRepository->getInactivePaginated(25, 'id', 'asc', $organization));
    }
     ...
     * @return mixed
     */
    public function inviteUsers(ManageUserRequest $request)
    {
        $organization = \Auth::user()->organizationUsers()->pluck('organization');
        // dd($organization);
        return view('backend.auth.user.invite-users')
            ->withUsers($this->userRepository->getNotInvitedUsers(25, 'id', 'asc', $organization));
    }
    ...
    public function getDeleted(ManageUserRequest $request)
    {
        ...
        return view('backend.auth.user.deleted')
            ->withUsers($this->userRepository->getDeletedPaginated(25, 'id', 'asc', $organization));
    }
    ...
    {
        $user = $this->userRepository->restore($deletedUser);
