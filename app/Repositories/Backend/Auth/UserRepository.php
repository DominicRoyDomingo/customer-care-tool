...
class UserRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function getActivePaginated($paged = 25, $orderBy = 'created_at', $sort = 'desc', $organization = null): LengthAwarePaginator
    {
        // dd(->toArray());
        if ($organization != null) {
            return $this->model
                ->with(['roles', 'permissions', 'providers', 'organizationUsers'])
     ...
     */
    public function getInactivePaginated($paged = 25, $orderBy = 'created_at', $sort = 'desc', $organization = null): LengthAwarePaginator
    {
        if ($organization != null) {
            return $this->model
                ->with('roles', 'permissions', 'providers', 'organizationUsers')
     ...
     * @return LengthAwarePaginator
     */
    public function getDeletedPaginated($paged = 25, $orderBy = 'created_at', $sort = 'desc', $organization = null): LengthAwarePaginator
    {
     ...
     * @return LengthAwarePaginator
     */
    public function getNotInvitedUsers($paged = 25, $orderBy = 'created_at', $sort = 'desc', $organization = null): LengthAwarePaginator
    {
        // dd($organization);
        return $this->model
            ->with('roles', 'permissions', 'providers', 'organizationUsers', 'notifications')
            ->whereHas('organizationUsers', function ($query) use ($organization) {
                return $query->whereNotIn('organization', $organization);
            })
     ...
     * @return mixed
     */
    public function confirmInvite($code, $organization)
    {
        $user = $this->confirmCode($code, $organization);
        // dd($user);
        if ($user->organizationUsers === true) {
            // dd('test');
    ...
    public function confirmCode($code, $organization)
    {
        $user = $this->model
        ...
        if ($user instanceof $this->model) {
            // dd($user->organizationUsers->where('organization', $organization)->isEmpty());
            if ($user->organizationUsers->where('organization', $organization)->isEmpty()) {
                return $user;
            }
