  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="card-header">
          ...
          <i class="fas fa-list-alt"></i> {{ $t( 'category_list' )}}

          <button type="button" class="btn btn-success btn-sm float-right"  @click="createTag()"><i class="fa fa-plus"></i> {{ $t( modal.add.button.new ) }} </button>
          ...
          <div class="col-md-12 mt-3">
            ...
            >
              ...
              <template v-slot:cell(name)="data">
                ...
                <span>{{data.item.name}} <small v-if="data.item.convertion==true" style="color:red">(No {{ data.item.language }} translation yet)</small></span>
      ...
      <Create :$this="this" @loadTable="loadCategory"></Create>
...
import Create from "./includes/category/CreateComponent.vue";

import Edit from "./includes/category/UpdateComponent.vue";
...
import { fetchList, softDelete, getCategoryName } from "./../api/category.js";
...
export default {
  ...
  methods: {
    ...
    onEdit(category) {
      ...
      getCategoryName( this.form.id, this.$i18n.locale ).then( resp => {
    ...
    onDelete(category) {
      ...
      this.$bvModal
        .msgBoxConfirm(
          this.$t( "question_record_delete") + `${category.name}` + this.$t( "from") +  this.$t( "category_name") + this.$t( "records"),
          {
        ...
        .then(resp => {
          ...
          if(resp) {
            ...
            softDelete(category.id).then(resp => {
              ...
              this.makeToast(
                ...
                resp.message + ' ' + this.$t( 'remove_category_alert' )
