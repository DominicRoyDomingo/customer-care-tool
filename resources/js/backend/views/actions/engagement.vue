<template>
  <div class="animated fadeIn">
    <v-app id="tags__container">
      <v-card tile flat>
        <v-card-title>
          <v-row>
            <v-col>
              <span class="title font-weight-light text--secondary">
                {{ $t("label.engagements") }}
              </span>
            </v-col>
            <v-spacer> </v-spacer>
            <v-col>
              <v-btn
                color="success"
                 v-b-modal.engagement-modal
                 @click="onAdd"
                class="float-right"
                tile
              >
                <v-icon dark>mdi-newspaper-plus</v-icon>
                <!-- <v-icon small dark style="margin-top: 15px; margin-left: -3px;">
                  mdi-plus
                </v-icon> -->
                &nbsp;
                {{ $t("button.new") }} {{ $t("label.engagements") }}
              </v-btn>
            </v-col>
          </v-row>
        </v-card-title>
        <v-card-text>
          <v-row>
            <v-col cols="12" sm="12" md="2">
              <v-row no-gutters>
                <v-col
                  cols="3"
                  class="caption font-weight-regular text--secondary text-right"
                  style="display:flex; justify-content: center; padding-top: 5px"
                >
                  {{ $t("button.show") }}
                </v-col>
                <v-col cols="6">
                  <b-form-select
                    :options="showEntriesOpt"
                    v-model="perPage"
                    style="border-radius: 0"
                  />
                </v-col>
                <v-col
                  cols="3"
                  class="caption font-weight-regular text--secondary"
                  style="display:flex; justify-content: center; padding: 5px 0 0 5px"
                >
                  {{ $t("label.entries") }}
                </v-col>
              </v-row>
            </v-col>
            <v-spacer></v-spacer>
            <v-col cols="12" sm="12" md="4">
              <b-input-group class="float-right">
                <template v-slot:prepend>
                  <b-input-group-text
                    style="background-color: rgba(0,0,0,0.05); 
                      border-radius: 0;"
                  >
                    <v-icon size="21">mdi-magnify</v-icon>
                  </b-input-group-text>
                </template>
                <b-form-input
                  v-model="filter"
                  :placeholder="$t('search_here')"
                  type="search"
                  style="border-radius: 0; border-left: 0"
                ></b-form-input>
              </b-input-group>
            </v-col>
          </v-row>
          <v-row>
            <v-container fluid>
              <b-table
                striped
                ref="table"
                show-empty
                :fields="tableHeaders"
                :current-page="currentPage"
                :per-page="perPage"
                :filter="filter"
                :busy="isLoading"
                :items="items"
              >

              <template v-slot:table-busy>
                <div class="d-flex justify-content-center p-2">
                  <b-spinner label="Small Loading..."></b-spinner>
                </div>
              </template>
              <template v-slot:cell(index)="data">
                <div style="margin-top: 10px">
                  {{ data.index + 1 }}
                </div>
              </template>
              <template v-slot:cell(engagement_name)="data">
                <div style="margin-top: 10px">
                  <span v-html="data.item.engagement_name"></span>
                </div>
              </template>
              <template v-slot:cell(brands)="data">
                <div style="margin-top: 10px">
                  <span v-html="data.item.engagement_brand_names"></span>
                </div>
              </template>
              <template v-slot:cell(created_at)="data">
                <div style="margin-top: 10px">
                  <span>{{ data.item.created_at | toLocaleString }}</span>
                </div>
              </template>

                <template v-slot:cell(actions)="data">
                  <div style="margin-top: 10px">
                    <v-menu offset-y>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn
                          color="#c8ced3"
                          light
                          v-bind="attrs"
                          v-on="on"
                          tile
                          depressed
                          small
                        >
                          {{ $t("button.more_actions") }}
                          <v-icon small right>
                            mdi-chevron-down
                          </v-icon>
                        </v-btn>
                      </template>
                      <v-list dense class="profile__row-menu">
                        <v-list-item-group color="primary">
                          <v-list-item @click="onEdit(data.item)">
                            <v-list-item-icon>
                              <v-icon color="yellow darken-3">
                                mdi-map-marker-check
                              </v-icon>
                            </v-list-item-icon>
                            <v-list-item-content>
                              <v-list-item-title>
                                {{ $t("button.edit") }}
                              </v-list-item-title>
                            </v-list-item-content>
                          </v-list-item>
                          <v-list-item @click="onDelete(data.item)">
                            <v-list-item-icon>
                              <v-icon color="red lighten-1">
                                mdi-map-marker-remove
                              </v-icon>
                            </v-list-item-icon>
                            <v-list-item-content>
                              <v-list-item-title>
                                {{ $t("button.delete") }}
                              </v-list-item-title>
                            </v-list-item-content>
                          </v-list-item>
                        </v-list-item-group>
                      </v-list>
                    </v-menu>
                  </div>
                </template>

                <template slot="empty">
                  <section class="section">
                    <div class="content has-text-grey has-text-centered">
                      <center>{{ $t("table.empty") }}</center>
                    </div>
                  </section>
                </template>
              </b-table>
            </v-container>
          </v-row>
          <v-row>
            <v-spacer></v-spacer>
            <v-col>
              <b-pagination
              v-model="currentPage"
              :total-rows="totalRows"
              :per-page="perPage"
            ></b-pagination>
            </v-col>
          </v-row>
        </v-card-text>
      </v-card>
      <EngagementModal :parent="this"></EngagementModal>
      <BrandModal :parent="this" @loadTable="loadBrand" @hide="onHideBrandModal"></BrandModal>
    </v-app>
  </div>
  <!-- <div class="animated fadeIn">
    <div class="card">
      <div class="card-body">
        <div class="row">
          <div class="col-md-6">
            <h4 class="card-title text-muted">{{ $t("label.engagements") }}</h4>
          </div>
          <div class="col-md-6">
            <b-button
              variant="primary"
              v-b-modal.engagement-modal
              class="float-right mb-3"
              @click="onAdd"
            >
              <i class="fa fa-plus" aria-hidden="true"></i>
              {{ $t("button.new") }}
            </b-button>
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-md-9">
            <div class="float-left">
              <div class="form-inline">
                <label class="mr-sm-2" for="inline-form-custom-select-pref">{{
                  $t("button.show")
                }}</label>
                <b-form-select
                  id="inline-form-custom-select-pref"
                  class="mb-2 mr-sm-2 mb-sm-0"
                  :options="showEntriesOpt"
                  v-model="perPage"
                />
                {{ $t("label.entries") }}
              </div>
            </div>
          </div>
          <div class="col-md-3">
            <b-input-group>
              <template v-slot:append>
                <b-input-group-text>
                  <i class="fa fa-search" aria-hidden="true"></i>
                </b-input-group-text>
              </template>
              <b-form-input
                v-model="filter"
                type="search"
                :placeholder="$t( 'search_here' )"
              ></b-form-input>
            </b-input-group>
          </div>
          <div class="col-md-12 mt-3">
            <b-table
              striped
              ref="table"
              show-empty
              :fields="tableHeaders"
              :current-page="currentPage"
              :per-page="perPage"
              :filter="filter"
              :busy="isLoading"
              :items="items"
            >
              <template v-slot:table-busy>
                <div class="d-flex justify-content-center p-2">
                  <b-spinner label="Small Loading..."></b-spinner>
                </div>
              </template>
              <template v-slot:cell(index)="data">
                {{ data.index + 1 }}
              </template>
              <template v-slot:cell(engagement_name)="data">
                <span v-html="data.item.engagement_name"></span>
              </template>
              <template v-slot:cell(brands)="data">
                <span v-html="data.item.engagement_brand_names"></span>
              </template>
              <template v-slot:cell(created_at)="data">
                <span>{{ data.item.created_at | toLocaleString }}</span>
              </template>
              <template v-slot:cell(actions)="data">
                <span>
                  <b-button
                    size="sm"
                    variant="success"
                    @click="onEdit(data.item)"
                  >
                    <i class="fas fa-edit"></i>
                    {{ $t("button.edit") }}
                  </b-button>
                  <b-button
                    size="sm"
                    variant="danger"
                    @click="onDelete(data.item)"
                  >
                    <b-spinner
                      v-if="data.item.is_loading"
                      small
                      label="Small Spinner"
                      type="grow"
                    ></b-spinner>
                    <span v-else>
                      <i class="fas fa-trash"></i>
                      {{ $t("button.delete") }}
                    </span>
                  </b-button>
                </span>
              </template>
            </b-table>
          </div>
          <div class="col-md-12">
            <b-pagination
              v-model="currentPage"
              :total-rows="totalRows"
              :per-page="perPage"
            ></b-pagination>
          </div>
        </div>
        <EngagementModal :parent="this"></EngagementModal>
        <BrandModal :parent="this" @hide="onHideBrandModal"></BrandModal>
      </div>
    </div>
  </div> -->
</template>

<script>
import { mapActions, mapGetters } from "vuex";
import engMixins from "./mixins/engagementMixins.js";
import EngagementModal from "./modals/engagement.modal.vue";
import BrandModal from "./modals/brand.modal.vue";

export default {
  mixins: [engMixins],
  components: { EngagementModal, BrandModal },
  data() {
    return {
      tableHeaders: [
        {
          key: "index",
          label: "SL",
          thStyle: { width: "10%" },
        },
        {
          key: "engagement_name",
          label: this.$t("label.engagement"),
          thClass: "text-left",
          thStyle: { width: "30%" },
          sortable: true,
        },
        {
          key: "brands",
          label: this.$t("brands_name"),
          thClass: "text-left",
          thStyle: { width: "20%" },
        },
        {
          key: "created_at",
          label: this.$t("table.created_at"),
          thClass: "text-right",
          thStyle: { width: "20%" },
          tdClass: "text-right",
        },
        {
          key: "actions",
          label: this.$t("table.action"),
          thClass: "text-center",
          thStyle: { width: "20%" },
          tdClass: "text-center",
        },
      ],
    };
  },

  created() {
    this.loadItems().catch((errors) => {
      console.log(errors);
    });
  },

  computed: {
    ...mapGetters({
      items: "actions/get_engagament_items",
      engageStatus: "actions/get_engagament_status"
    }),
    ...mapGetters("brand", {
      brands: "brands",
    }),
  },

  methods: {
    ...mapActions("actions", ["get_engagaments", "delete_engagement", "remove_from_engagements_array"]),
    ...mapActions("brand", ["get_brands"]),
    async loadItems() {
      this.isLoading = true;
      let params = {
        api_token: this.$user.api_token,
        locale: this.$i18n.locale,
      };

      this.get_engagaments(params).then((_) => {
        this.isLoading = false;
      });
    },

    loadBrand() {
      let params = {
        api_token: this.$user.api_token,
        lang: this.form.language,
        entries: this.perPage,
        page: this.currentPage,
        sort_name: this.sortBy,
        sort_desc: this.sortDesc,
        search: this.searched,
        org_id: this.$org_id
      };
      this.get_brands(params).then((_) => {
      });
    },
    
    modalAddNewBrand(from) {
      this.form.previous_action = this.form.action;
      this.form.action = "Add";
      this.resetBrandForm();
      this.btnloading = false;

      this.$bvModal.hide("engagement-modal");
      this.$bvModal.show("brand-modal");
    },

    addedNewBrandSuccessfully() {
      this.form.action = this.form.previous_action;
      this.fetchBrands();
      this.resetBrandForm();
      this.$bvModal.show("engagement-modal");
    },
    
    onHideBrandModal(){
      this.$bvModal.hide("brand-modal");
      this.$bvModal.show("engagement-modal");
    },

    onAdd() {
      this.form.reset();
      this.btnloading = false;
      this.form.action = "Add";
    },
    onEdit(item) {
      this.form.reset();
      this.editingIndex = item.engagement_index;
      this.form.id = item.id;
      this.form.engagement =
        this.$user.locale == "en" ? item.is_english : item.is_italian;
      this.form.brands = JSON.parse(item.brands);
      this.form.action = "Update";
      this.form.default = item.engagement_name;

      this.itemSelected = item;

      // Open Modal
      this.$bvModal.show("engagement-modal");
    },
    successfullySavedEngagement(){
      this.$refs.table.refresh();
    },
    onDelete(item) {
      let vm = this;
      let engagement = item;
      
      item.is_loading = true;

      this.$bvModal
        .msgBoxConfirm(
           this.$t( "question_record_delete") + `${item.engagement_name}` + this.$t( "from") +  this.$t( "label.engagement") + this.$t( "records"),
          {
            title: this.$t( "confirmation_record_delete"),
            okVariant: "danger",
            okTitle: this.$t( "yes"),
            cancelTitle: this.$t( "no"),
            hideHeaderClose: false,
            centered: true
          }
        )
        .then((value) => {
          if (value) {
            item.is_loading = true;
            let params = {
              api_token: this.$user.api_token,
              locale: this.language,
              id: item.id,
            };

            this.delete_engagement(params)
              .then((resp) => {
                item.is_loading = false;
                if (this.engageStatus) {
                  vm.remove_from_engagements_array(engagement);
                  this.makeToast(
                    "danger",
                    this.$t( "delete_record"),
                    `${item.engagement_name}` + this.$t( "delete_record_message") + this.$t( "from") +  this.$t( "label.engagement") + this.$t( "records")
                  );
                }
              })
              .catch((error) => {
                this.form.errors.record(error.response.data.errors);

                this.makeToast(
                  `danger`,
                  this.$t( "inused_alert"),
                  this.$t( "unable_message1") + `${item.engagement_name}` + this.$t( "unable_message2" ),
                  `${item.engagement_name} ` + this.$t( "delete_record_message") + this.$t( "from") +  this.$t( "label.engagement") + this.$t( "records")
                );
                item.is_loading = false;
              });
          }
        })
        .catch((err) => {
          // An error occurred
        });
    },
  },
};
</script>
