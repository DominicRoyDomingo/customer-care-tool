                  ...
                  <v-container>
                    <v-row>
                      ...
                      <b-form-group style="margin-bottom:1%">
                        ...
                        <small style="color:red; margin-top:-15px;position:absolute" v-if="engagement_required"> {{ $t('label.engagement') }} {{ $t('is_required')}}</small>
                      </b-form-group>
                      </v-col>
                    </v-row>
                    <v-row>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      brands: "actions/get_brand_items",
    }),
  },
  ...
  methods: {
    ...
    close() {
      this.engagement_required = false;
      this.$bvModal.hide("engagement-modal");
    },
    onSave() {
      this.engagement_required = false;
      if( this.parent.form.engagement == '' ){
        this.engagement_required = true;
        return false;
      }
      let vm = this;
      ...
      this.post_engagement(params)
        .then((resp) => {
          ...
          this.$bvModal.hide("engagement-modal");
          if (this.engageStatus.responseStatus) {
            //this.parent.loadItems();

            console.log("engageStatus");
            console.log(this.engageStatus);

            if (this.parent.form.action == "Add") {
              this.add_engagement(this.engageStatus);
            } else {
              let engagement = vm.engageStatus;
              engagement.engagement_index = vm.parent.editingIndex;
              vm.update_engagement(engagement);
            }
            ...
            this.parent.successfullySavedEngagement();
          }
        })
        .catch((error) => {
