...
export default {
  ...
  data: function() {
    return {
      modal: this.parent.modal.createAlgoliaPermission,
  ...
  methods: {
    ...mapActions("algolia_permission", ["post_algolia_permission", "add_algolia_permission"]),
    
    modalClose(done) {
      this.removeErrors();
      this.parent.$bvModal.hide("algolia-permission-add-modal");
    ...
    onSubmit() {
      ...
      this.post_algolia_permission(params)
        .then((resp) => {
          this.removeErrors();
          this.$bvModal.hide("algolia-permission-add-modal");
					let message = {
						create:
							this.$t("created_message_algolia"),
						title: {
