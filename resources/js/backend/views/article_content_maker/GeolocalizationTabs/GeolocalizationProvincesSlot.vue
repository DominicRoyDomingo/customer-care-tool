            ...
            <template v-slot:cell(count_providers)="data">
              <div style="margin-top: 10px">
                <span class="text--disabled">
                  	<a href="#" class="font-weight-bold" @click.prevent="onShowProvider(data.item, data.index)">
...
import Edit from "../../includes/geolocalization/provinces/UpdateComponent.vue";
...
export default {
  ...
  methods: {
    ...mapActions("geolocalization", ["get_geolocalization_provinces", "delete_geolocalization", "remove_from_geolocalization_array"]),
    ...mapActions("geolocalization_template", ["get_geolocalization_template"]),
    ...mapActions("country", ["get_all_province"]),
    ...
    async loadItems() {
      ...
      this.get_geolocalization_provinces(params).then(_ => {
    ...
    onEdit(item,index) {
      ...
      this.place = item
      this.loadPlaces();
      this.$bvModal.show("geolocalization-province-edit-modal");
    },
  ...
  computed: {
    ...mapGetters("geolocalization", {
      items: "geolocalization_provinces",
