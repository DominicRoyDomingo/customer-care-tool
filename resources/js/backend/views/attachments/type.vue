<template>
  <div class="animated fadeIn">
    <v-app id="type__container">
      <v-card tile flat>
        <v-card-title>
          <v-row>
            <v-col>
              <div class="title font-weight-light text--secondary">
                {{ $t("label.type_of_document") }}
              </div>
            ...
            <v-col>
              ...
              >
                ...
                <v-icon size="13" style="margin: 10px 0 0 -4px">
                  mdi-plus
                </v-icon>
                &nbsp;
                {{ $t("label.new_type") }}
          ...
          <v-row>
            <v-container fluid>
              ...
              >
                ...
                <template v-slot:cell(document_type_name)="data">
                  <div style="margin: 10px 0 0 25px;">
                    <span
                      class="text-left text--secondary"
                    >
                      {{ data.item.document_type_name }}
                      <small
                    ...
                    <!-- <span
                      ...
                      v-html="data.item.document_type_name"
                    >
                    </span> -->
                ...
                <template
                  v-slot:cell(document_category.document_category_name)="data"
                >
                  <div style="margin: 10px 0 0 25px;">
                    <span
                      class="text-left text--secondary"
                    >
                      {{ data.item.document_category.document_category_name }}
                      <small
                        v-if="data.item.convertion_document_category == true"
                        style="color:red"
                      >
                        (No {{ data.item.language }} translation yet)</small
                      >
      ...
      <categoryFormModal :parent="this"></categoryFormModal>
    </v-app>
  </div>
</template>
...
import formModal from "./modals/type/formModal";
import categoryFormModal from "./modals/category/formModal";
...
export default {
  ...
  data() {
    return {
      tableHeaders: [
        ...
        {
          key: "document_category.document_category_name",
  ...
  methods: {
    ...mapActions("attachments", ["get_types", "delete_type"]),
    async loadTypes() {
      this.isLoading = true;
      ...
      await this.get_types(params).then((_) => {
    ...
    openFormModalAsEdit(item, index) {
      ...
      this.form.category_name = item.document_category.id;
      this.form.name = item.document_type_name.includes("<small")
        ? null
    ...
    onDelete(item, index) {
      ...
      let confirmation_message = vm.$t("label.delete_type_confirmation");
      confirmation_message = confirmation_message.replace(
        /%variable%/g,
        "<strong>" + item.document_type_full_name + "</strong>"
      );
            ...
            action: function(value) {
              if (value) {
                ...
                let notification_message = vm.$t(
                  "toasts.deleted_document_type"
                );
                notification_message = notification_message.replace(
                  /%variable%/g,
                  item.document_type_full_name
                );

                vm.delete_type(params)
                  .then((resp) => {
                  ...
                  .catch((error) => {
                    vm.makeToast(
                      ...
                      error.response.data.errors.documentType[0]
                    );
  ...
  watch: {
    isTypeFormModalOpen: function(isShown) {
