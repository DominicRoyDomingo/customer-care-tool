                  ...
                  <v-col cols="12" md="12" class="modal__input-container">
                    <label :for="'brands'">
                        ...
                        <strong class="text-danger">*</strong>
										...
										<v-select
											label="place"
											:options="parent.parent.regionPlaces"
											:filterable="false"
											v-model="form.places"
											multiple
											@open="onOpen"
											@close="onClose"
											@search="query => parent.parent.regionSearchPlace = query"
										>
...
export default {
  ...
  data: function() {
    return {
      modal: this.parent.geolocalizationRegion.createGeolocalization,
	...
	computed: {
		filtered () {
			...
			return this.parent.parent.regionPlaces.filter(place => place.place.toLowerCase().includes(this.search.toLowerCase()));
		},
		paginated () {
			return this.filtered.slice(0, this.limit);
		},
		hasNextPage () {
			return this.parent.parent.regionPlacesLength != 0;
		},
	},
  ...
  methods: {
    ...
    modalClose(done) {
      ...
      this.parent.$bvModal.hide("geolocalization-region-add-modal");
    ...
    async infiniteScroll ([{isIntersecting, target}]) {
      if (isIntersecting) {
        setTimeout(() => {
          ...
          this.parent.parent.regionPlacePage = this.parent.parent.regionPlacePage+1;
          let params = {
          ...
          this.parent.parent.get_all_region(params).then((data) => {
      ...
      this.post_geolocalization(formData)
        .then((resp) => {
          ...
          if (this.parent.parent.geolocalizationResponseStatus) {
            ...
            let message = {
              create:
                `${this.parent.parent.geolocalizationResponseStatus} ${this.$t("label.regions").toLowerCase()} ${this.$t("brand_linked1")} '${this.parent.parent.parent.article.base_name}'`,
              title: {
