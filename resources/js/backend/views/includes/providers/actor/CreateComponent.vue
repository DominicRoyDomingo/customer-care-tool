                ...
                <v-container>
                  ...
                  <v-row>
                    <v-col cols="12" md="12" class="modal__input-container">
                      <label for="field_of_profession"
                        >{{ $t("label.field_of_profession") }}:</label
                      >
                      <v-select
                        name="name"
                        label="base_name"
                      ...
                      >
                        ...
                        <v-tab-item eager>
                          <!-- <v-fade-transition>
                            <v-overlay
                              v-if="
                                hover &&
                                parent.form.specializationCategories.length == 0
                              "
                              absolute
                              color="#282828"
                              opacity="0.9"
                            >
                              <h4>{{ $t("actor_overlay") }}</h4>
                            </v-overlay>
                          </v-fade-transition> -->
                          <v-container>
                            ...
                            <!-- <div
                              ...
                              v-for="(specialization, index) in parent.form
                                .specializations"
                              v-bind:key="'specialization_' + index"
                            >
                              ...
                              <div class="form-group">
                                ...
                                <v-select
                                  ...
                                  v-model="
                                    parent.form.specializations[index]
                                      .profession
                                  "
                                  :options="parent.newSpecializations"
                                  @input="parent.removeDuplicateSpecializations"
                                  id="specialization"
                                >
                                  <template #list-header>
                                    <li style="margin-left: 20px" class="mb-2">
                            ...
                            <!-- <specializationDiv
                                    v-for="(specialization, index) in parent.new_specializations"
                                    v-bind:key="'specialization_' + index"
                                    :root_parent="parent"
                                    :parent="vm"
                                    :index="specialization.index"
                                    :is_deletable="true"
                                    ref="specializationDiv"
                                ></specializationDiv> -->
                            <div class="row">
                              <div class="col-md-12">
                                <v-btn
                                  ...
                                  @click.prevent="parent.addSpecializationDiv"
                                >
                                  <v-icon>mdi-sticker-plus</v-icon>&nbsp;
                                  {{ $t("label.add_specialization") }}
                                </v-btn>
                              </div>
                            </div>
                          </v-container>
                        ...
                        <v-tab-item eager>
                          <v-container>
                            <div
                              class="note-div note-data"
                              v-for="(physical_code, index) in parent.form
                                .physical_codes"
                            ...
                            >
                              <div class="row no-gutters">
                                ...
                                <div class="col-md-1 text-right">
                                  <v-btn
                                    ...
                                    small
                                    color="error"
                                    @click.prevent="deletePhysicalCode(index)"
                                  >
                                    <v-icon dark>mdi-close</v-icon>
                                  </v-btn>
                                </div>
                              ...
                              <div class="form-group">
                                <label for="physical_code_type">{{
                                  $t("label.physical_code_type")
                                }}</label>
                                <v-select
                                  name="name"
                                  label="physical_code_type_name"
                                  v-model="
                                    parent.form.physical_codes[index]
                                      .physical_code_type
                                  "
                                ...
                                >
                                  <template #list-header>
                                    <li style="margin-left: 20px" class="mb-2">
                                      <b-link
                                        href="#"
                                        @click="openPhysicalCodeTypeModal"
                                      >
                                        <i
                                          class="fa fa-plus"
                                          aria-hidden="true"
                              ...
                              <div class="form-group">
                                <label for="physical_code">{{
                                  $t("table.physical_code")
                                }}</label>
                                <b-form-input
                                  v-model="
                                    parent.form.physical_codes[index].code
                                  "
                              ...
                              <div class="row no-gutters">
                                ...
                                <div class="col-md-1 text-right">
                                  <v-btn
                                    ...
                                    small
                                    color="error"
                                    @click.prevent="deleteOtherInfoDiv(index)"
                                  >
                                    <v-icon dark>mdi-close</v-icon>
                                  </v-btn>
                                </div>
...
export default {
  ...
  data: function () {
    return {
      modal: this.parent.modal.createActor,
  ...
  methods: {
    ...mapActions("actor", ["post_actor", "add_actor"]),
    ...mapActions("categ_terms", ["get_terms_descriptions", "get_terms"]),


    modalClose(done) {
      this.$emit("on-close"); // for article event please dont delete.

      this.parent.$bvModal.hide("actor-add-modal");

      this.parent.$bvModal.show("link-actor-modal");
    ...
    onSubmit() {
      ...
      this.post_actor(params)
        .then((resp) => {
          this.$bvModal.hide("actor-add-modal");

          if (this.parent.actorsResponseStatus) {
            let message = {
              create:
                `${this.parent.actorsResponseStatus.fullname}` +
                this.$t("created_message") +
                this.$t("successfull_actor"),
              title: {
    ...
    async loadTerms() {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.totalRow = this.filter ? items.data.length : items.total;
        })
        .catch((error) => {
          console.log(error);
        })
