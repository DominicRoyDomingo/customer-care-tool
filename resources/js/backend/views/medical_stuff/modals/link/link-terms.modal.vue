...
export default {
  ...
  data() {
    return {
      ...
      parent_id: this.parent.mtForm.id,
  ...
  mounted() {
    this.load_term_descriptions().catch((error) => {
      console.log(error);
    });
  ...
  methods: {
    ...
    async load_term_descriptions() {
      ...
      this.get_terms_descriptions(params).then((_) => {
        ...
        if (this.termDescForm.action === "Add") {
          this.terms.forEach((ele) => {
            if (this.on_isLinked(ele.id)) {
              ele.is_rendered = false;
              this.$nextTick(() => {
                ele.is_rendered = true;
              });
            }
          });
        }
    ...
    on_search_submit() {
      this.isOverlay = true;
      let linkedTerms = "";
      if (this.parent.linkedMedicalTerm.length > 0) {
        linkedTerms = JSON.stringify(this.parent.linkedMedicalTerm);
      }

      this.linked_medical_terms = linkedTerms;
      this.load_terms().catch((error) => {
        console.log(error);
      });
    },
